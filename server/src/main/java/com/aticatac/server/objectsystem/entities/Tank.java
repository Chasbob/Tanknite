...
public class Tank extends Entity implements DependantTickable<CommandModel>, Hurtable {
  protected final ConcurrentLinkedQueue<CommandModel> frames;
  //  protected final Entity entity;
  protected final Logger logger;
  protected final PlayerOutputService outputService;
  private final Physics physics;
  //  protected Position position;
  protected CommandModel input;
...
//    entity = new Entity(name, EntityType.TANK);
    ...
    input = new CommandModel("", Command.DEFAULT);
//    position = p;
...
//    this.box = new CollisionBox(position, EntityType.TANK);
  ...
  public void tick() {
    ...
    if (!frames.isEmpty()) {
      input = frames.poll();
      this.logger.trace(input.getCommand());
      try {
        if (health > 10) {
          move(input.getVector(), speedIncrease);
        }
      } catch (Exception e) {
        this.logger.error(e);
    ...
    if (input.getCommand() == Command.SHOOT) {
      this.logger.info("shoot");
      if (!(ammo == 0 || health == 0) && framesToShoot < 0) {
        setAmmo(ammo - 1);
        if (damageIncrease) {
          outputService.addBullet(new Bullet(getBaseEntity(), position.copy(), input.getBearing(), 20));
        } else {
          outputService.addBullet(new Bullet(getBaseEntity(), position.copy(), input.getBearing(), 10));
        }
        framesToShoot = 60;
      }
    }
...
//  }
   ...
   void move(Vector vector, final boolean speedIncrease) {
    if (!vector.equals(Vector.Zero)) {
      var response = physics.move(vector.angle(), position, speedIncrease);
